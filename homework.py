...
class Training:
    ...
    def __init__(
        ...
        self.weight = weight
        self.training_time_in_minutes = self.duration * self.TIME_IN_MINUTES
    ...
    def show_training_info(self) -> InfoMessage:
        ...
        string_massage = (f'Тип тренировки: {self.__class__.__name__}; '
...
class SportsWalking(Training):
    ...
    def get_spent_calories(self) -> float:
        """Получить количество затраченных калорий."""
        meters_per_second = ((self.get_mean_speed() * self.M_IN_KM) / self.SECONDS_PER_HOUR)**2
        result_spent_calories = (
                                (self.RATIO_WALKING_ONE * self.weight +
                                (meters_per_second / self.height) *
                                self.RATIO_WALKING_TWO * self.weight) *
                                self.training_time_in_minutes
                                )  
        return result_spent_calories
...
class Swimming(Training):
    ...
    def get_distance(self) -> float:
        """Получить дистанцию в км."""
        result_distance_swimming = (self.action * self.LEN_HAND_SWING) / self.M_IN_KM
        return result_distance_swimming
    
    def get_mean_speed(self):
        result_mean_speed_swimming = ((self.length_pool * self.count_pool ) / self.M_IN_KM) / self.duration 
        return result_mean_speed_swimming
    
    def get_spent_calories(self):
        result_spent_calories_swing = (
            (self.get_mean_speed() + 
             self.CALORIE_LOSS_RATIO_WHEN_SWIMMING_ONE) * 
